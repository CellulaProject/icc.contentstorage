...
__MIME_DATA__ = """3D Crossword Plugin 	application/vnd.hzn-3d-crossword 	.x3d 	IANA: 3D Crossword Plugin
...
for l in __MIME_DATA__.split("\n"):
    name, mimetype, ext, url = [e.strip()
                                for e in (l.split("\t") + [""] * 4)[:4]]
    # print("{}|{}|{}|{}".format(name, mimetype, ext, url))
    if ext and mimetype:
        MIME_TO_EXT[mimetype] = ext
